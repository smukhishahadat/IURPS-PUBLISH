$.fn.flashMessage = function (options) {
    var target = this;
    options = $.extend({}, options, { timeout: 9000, alert: options });
    ...
    if (options.message) {
        $(target).addClass("alert-solid-" + options.alert.toString().toLowerCase());

        if (typeof options.message === "string") {
            $('.alert-text', target).html("<span>" + options.message + "</span>");
        } else {
